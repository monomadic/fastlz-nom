            ...
            match o {
                ...
                Offset::Literal { length } => {
                    if let Ok((r, bytes)) = take_bytes(rem, length) {
                        ...
                        log::error!("error: cannot take any more literal bytes, reached end of compressed buffer.");
                        break;
                    }
                }
            }
...
mod tests {
    ...
    #[test]
    fn test_deflate() {
        let input = include_bytes!("../tests/data/compressed");
        let output = include_bytes!("../tests/data/decompressed");
        let (_, input) = deflate(input, 0).expect("deflate failed");
